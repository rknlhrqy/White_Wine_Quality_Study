...
#Before study, turn off unnecessary messages.
...
ggplot(aes(x = quality), data = wine) +
  #geom_bar() is for categorical data.
  #geom_histogram() is for continuous data.
  geom_bar(color = 'black', fill = I('#099009')) +
...
ggplot(aes(x = residual.sugar), data = wine) +
  geom_histogram(binwidth = 0.04, color = 'black', fill = I('#099009')) +
...
ggplot(aes(x = fixed.acidity), data = wine) +
  geom_histogram(binwidth = 0.2, color = 'black', fill = I('#099009')) +
...
```{r}
ggplot(aes(x = volatile.acidity), data = wine) +
  geom_histogram(binwidth = 0.05, color = 'black', fill = I('#099009')) +
...
ggplot(aes(x = free.sulfur.dioxide), data = wine) +
  geom_histogram(binwidth = 2, color = 'black', fill = I('#099009')) +
...
ggplot(aes(x = total.sulfur.dioxide), data = wine) +
  geom_histogram(binwidth = 4, color = 'black', fill = I('#099009')) +
  scale_x_continuous(breaks = seq(0, 440, 50)) +
...
```
In order to clearly show the relationship between different features, here I  split the features into several different plots. Since this study is to find out how Quality is related to other features, in each plot, Quality is plotted.

We can see the correlation coefficients between Quality and other features in the plots below. In order to evaluate these correlation coefficients, here I use the popular guidelines:
...
ggpairs(pair, lower = list(continuous = wrap("points", shape = I('.'))),
        upper = list( combo = wrap("box", outlier.shape = I('.'))))
```

In the plot above, we can see Density and Alcohol have the moderate correlation with Quality. Residual Sugar has a weak correlation instread. 
Among these features, Density has very strong correlation with Alcohol and Residual Sugar. The reason of this strong correlation is studied later.
...
In the plot above, Citric Acid, Fixed Acidity, Volatile Acidity, and pH all have weak correlation with Quality. However among these features, pH has a moderate correlation with Fixed Acidity. 
...
In the plot above, Total Sulfur Dioxide, Free Sulfur Dioxide, Sulphates, and Chlorides all have the weak correlation with Quality.
However Free Sulfur Dioxide has a nearly strong correlaton with Total Sulfur Dioxide. 
...
I want to look closer at the boxplot involving Quality and Density, as well as Alcohol.

Before plotting, I need to create a function wine_quality_boxplot(). This function will draw a boxplot to show the relationship between Quality of wines and any other feature.

```{r}
library(ggplot2)
wine_quality_boxplot <- function(dataset, feature, feature_title_string) {
  # Use the functions deparse() and substitute() to create colume
  # label which are character string version of the actual
  # arguments to the function remove_outliers.
  feature = deparse(substitute(feature))
  ggplot(aes(y = dataset[[feature]], x = quality, group=quality),
       data = dataset) + 
    geom_boxplot(color = 'blue', fill = I('#099009'),
               outlier.size = 3, outlier.colour = 'black') +
    geom_smooth(method = "lm", color = "red",
              fill = "yellow", se = TRUE, aes(group=1)) +
    xlab("Quality") +
    # Add Feature's title. It is a string
    # for example, "Density (Kg/L)"
    ylab(feature_title_string)     
}
```

Before drawing the boxplot, I need to group the dataset using Quality.
```{r}
library(dplyr)
wine_group_quality<-wine %>%
  group_by(quality)
```

Here is the boxplot of Alcohol vs Quality.
```{r}
wine_quality_boxplot(wine_group_quality, alcohol, "Alcohol (%)")
```
...
```{r}
wine$alcohol_cate <-
  cut(wine$alcohol, seq(7.5, 15.5, 1), labels=c(8:15), right=FALSE)
```
...
Here is the boxplot of Density vs Quality.

```{r}
wine_quality_boxplot(wine_group_quality, density, "Density (Kg/L)")
```
...
It uses the popular algorithm of detecting outliers: data not inside the range [Q1 - 1.5*IQR, Q3 + 1.5*IQR].
```{r}
library(dplyr)
remove_outliers <- function (dataset, feature) {
  ...
  # arguments to the function remove_outliers.
  feature <- deparse(substitute(feature))
  # Calculate IQR (Inter Quartile Range) which the different
  # between 3rd and 1st quartiles
  rng <- (quantile(dataset[[feature]], probs=c(.25, .75))[2]
          - quantile(dataset[[feature]], probs=c(.25, .75))[1]) * 1.5
  ...
  # filter out all the data which is outside of the range.
  result_dataset <- dataset %>%
    dplyr::filter(high_bar >= dataset[[feature]]
                  & low_bar <= dataset[[feature]]) %>%
...
```{r}
wine_no_outlier_density_groupby_quality <-
  wine_no_outlier_density %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_density_groupby_quality,
...
```{r}
wine_no_outlier_density$density_cate <-
  cut(wine_no_outlier_density$density, seq(0.985, 1.005, 0.005),
      labels=seq(0.985, 1.000, 0.005), right=FALSE)
```
...
```{r}
wine_quality_boxplot(wine_group_quality,
...
```{r}
wine_no_outlier_residual_sugar_group_quality <-
  wine_no_outlier_residual_sugar %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_residual_sugar_group_quality,
                     residual.sugar, 'Residual Sugar (g/L)')
```
...
```{r}
wine_no_outlier_density_residualsugar <-
  remove_outliers(wine_no_outlier_density, residual.sugar)
```

Convert it to be Categorical.
```{r}
wine_no_outlier_density_residualsugar$residual.sugar_cate <-
  cut(wine_no_outlier_density_residualsugar$residual.sugar,
      seq(0, 25, 5), labels=c(1:5)*5, right=FALSE)
```


```{r}
ggplot(aes(x = quality), data = wine_no_outlier_density_residualsugar) + 
  geom_density(aes(group = residual.sugar_cate, colour=residual.sugar_cate),
               size=1) +
  labs(color = "Residual Sugar (g/L)") +
  scale_x_continuous(lim=c(3, 9), breaks=seq(3, 9, 1))+
  xlab("Quality") +
  ylab("Density Distribution")
```
...
I am afraid that the outliers of Free Sulfur Dioxide are affecting the accuracy. I need to remove these outliers.

```{r}
wine_no_outlier_free_sulfur_dioxide <-
  remove_outliers(wine, free.sulfur.dioxide)
```

Redraw the plot.

```{r}
wine_no_outlier_free_sulfur_dioxide_group_quality <-
  wine_no_outlier_free_sulfur_dioxide %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_free_sulfur_dioxide_group_quality,
                     free.sulfur.dioxide, "Free Sulfur Dioxide (mg/L)")
```

The line modeling shows no clear relation between Free Sulfur Dioxide and Quality. We can say that it is very hard to see that Free Sulfur Dioxide in wines correlates with Quality. 
...
```{r}
wine_no_outlier_total_sulfur_dioxide <-
  remove_outliers(wine, total.sulfur.dioxide)
```
...
```{r}
wine_no_outlier_total_sulfur_dioxide_group_quality <-
  wine_no_outlier_total_sulfur_dioxide %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_total_sulfur_dioxide_group_quality,
                     total.sulfur.dioxide, "Total Sulfur Dioxide (mg/L)")
```
...
```{r}
wine_no_outlier_total_sulfur_dioxide$total.sulfur.dioxide_cate <-
  cut(wine_no_outlier_total_sulfur_dioxide$total.sulfur.dioxide,
      seq(20, 260, 20), labels=seq(30, 250, 20), right=FALSE)
```
...
ggplot(aes(x = quality), data = wine_no_outlier_total_sulfur_dioxide) +
  ...
  labs(color = "Total Sulfur Dioxide (mg/L)") +
  scale_x_continuous(lim=c(3, 9), breaks=seq(3, 9, 1))+
  xlab("Quality") +
  ylab("Density Distribution")
```

In this plot, when Quality is 7 and 8, Density distribution curves with Total Sulfur Dioxide from 70mg/L to 170mg/L have higher values than other Total Sulfur Dioxide values. So we can say that wines with high Quality tend to have the level of Total Sulfur Dioxide at around 70mg/L to 170mg/L. Too much or too less Total Sulfur Dioxide is affecting the Quality of wines negatively.

I added a new feature: Bound Sulfur Dioxide. Here I am going to check how it affects the quality of wines.

```{r}
wine_quality_boxplot(wine_group_quality,
                     bound.sulfur.dioxide, "Bound Sulfur Dioxide (mg/L)")
```
...
```{r}
wine_no_outlier_bound_sulfur_dioxide_group_quality <- 
  wine_no_outlier_bound_sulfur_dioxide %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_bound_sulfur_dioxide_group_quality,
                     bound.sulfur.dioxide, "Bound Sulfur Dioxide (mg/L)")
```

From the line modeling in the plot above, we can see that wines with higher Bound Sulfur Dioxide tend to have low quality.
...
```{r}
wine_no_outlier_chlorides_group_quality <- wine_no_outlier_chlorides %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_chlorides_group_quality,
                     chlorides, "Chlorides (g/L)")
```

The line modeling in the plot above shows that wines with higher chlorides tend to have low quality.
...
It appears that wines with higher quality tend to have a bit higher Sulphates. However there are some outliers. Let us remove them and check it again.



```{r}
wine_no_outlier_sulphates <- remove_outliers(wine, sulphates)
```

Redraw the plot.

```{r}
wine_no_outlier_sulphates_group_quality <- wine_no_outlier_sulphates %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_sulphates_group_quality,
                     sulphates, "Sulphates (g/L)")
```

Now we can see that the line modeling does not show a clear relation between Quality and Sulphates.
...
It appears that wines with higher quality tend to have a bit higher pH, i.e. less acid.  Let us remove the outliers and check it again.


```{r}
wine_no_outlier_pH <- remove_outliers(wine, pH)
```

Redraw the plot.

```{r}
wine_no_outlier_pH_group_quality <- wine_no_outlier_pH %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_pH_group_quality,
                     pH, "pH")
```

Now we can see the line modeling is showing a correlation between pH and Quality. Higher pH does bring higer Quality.
...
```{r}
wine_no_outlier_fixedacidity_group_quality <- wine_no_outlier_fixedacidity %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_fixedacidity_group_quality,
                     fixed.acidity, "Fixed Acidity (g/L)")
```

In the plot above, the line modeling shows Fixed Acidity correlates with Quality negatively and weakly. Wines with higher Fixed Acidity tend to have a bit low quality.
...
```{r}
wine_no_outlier_volatileacidity_group_quality <-
  wine_no_outlier_volatileacidity %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_volatileacidity_group_quality,
                     volatile.acidity, "Volatile Acidity (g/L)")
```

The line modeling in the plot above shows that the volatile acidity correlates with the quality of wines negatively and very weakly. Wines with higher Volatile Acidity tend to have a bit low quality.
...
```{r}
wine_quality_boxplot(wine_group_quality,
                     total.acidity, "Total Acidity (g/L)")
```
...
```{r}
wine_no_outlier_totalacidity_group_quality <-
  wine_no_outlier_totalacidity %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_totalacidity_group_quality,
                     total.acidity, "Total Acidity (g/L)")
```

The line modeling in the plot above shows that the total acidity affects the quality of wines a bit. The wines with high Total Acidity tend to have a bit low Quality.

To further study it, I am going to create the Density Distribution plot.

```{r}
wine_no_outlier_totalacidity$total.acidity_cate <-
  cut(wine_no_outlier_totalacidity$total.acidity,
      seq(4.5, 9.5, 1), labels=seq(5, 9, 1), right=FALSE)
```


```{r}
ggplot(aes(x = quality), data = wine_no_outlier_totalacidity) + 
  geom_density(aes(group = total.acidity_cate, colour=total.acidity_cate),
               size=1) +
  labs(color = "Total Acidity (g/L)") +
  scale_x_continuous(lim=c(3, 9), breaks=seq(3, 9, 1))+
  xlab("Quality") +
  ylab("Density Distribution")
```
...
```{r}
wine_no_outlier_citricacid_group_quality <- wine_no_outlier_citricacid %>%
  group_by(quality)

wine_quality_boxplot(wine_no_outlier_citricacid_group_quality,
                     citric.acid, "Citric Acid (g/L)")
```

The line modeling is almost flat in the plot above. It shows that the citric acid does not affect the quality of wines.
...
q3 <- ggplot(aes(x = citric.acid, y = chlorides),
             data = wine_no_outlier_citricacid) + 
  geom_jitter(alpha = 0.5, size = 1.5, color = "blue") +
  geom_smooth(method = "lm", color = "red") +
  ylim(0, 0.4) +
  xlim(0, 0.6) +
...
In the three plot above, the line modeling is almost flat. It means that the relation between Fixed Acidity/Volatile Acidity/Citric Acid and Chlorides are not clear. It does not appear that wine with more salt also has higher acids. So it does not show that salt is added into wine with high citric acid.
...
ggplot(aes(x = residual.sugar, y = alcohol),
       data = wine_no_outlier_residual_sugar) + 
...
The line modeling clearly shows a clear negative correlation between Alcohol and Quality. It verifies that the wine with higher Residual Sugar appears to have less alcohol level.
...
```{r}
wine_no_outlier_residual_sugar_highquality6 <-
  subset(wine_no_outlier_residual_sugar,
         wine_no_outlier_residual_sugar$quality >= 6)
``` 

And then we remove the outliers of Total Acidity and Residual Sugar.

```{r}
wine_no_outlier_residualsugar_totalacidity_highquality6 <-
  remove_outliers(wine_no_outlier_residual_sugar_highquality6,
                  citric.acid)
```
...
ggplot(aes(x = residual.sugar, y = total.acidity),
       data = wine_no_outlier_residualsugar_totalacidity_highquality6) + 
  geom_smooth(method = "lm", color = "red") +
  geom_point() +
  xlab("Residual Sugar (g/L)") +
...
The line modeling in the plot above is almost flat. It does not show a clear correlation between Residual Sugar and Total Acidity. 
We cannot see that high quality wines tends to have high Total Acidity if its Residual Sugar level is also high.
...
In the plot above, the line modeling shows a clear negative correlation between pH and Total Acidity. This indicates that high Total Acidity causes the lower pH value. This verifies the relationship between acids and pH value in wines mathematically. 
...
- Acid brings the taste of dry, which reduces the sweetness of the wine. However study shows that in higher (>=6) quality wines, high acids does not seem to have high Residual Sugar in wines.
...
```{r}
ggplot(wine_no_outlier_density_residualsugar, 
       aes(x = quality, fill = alcohol_cate)) +
...
And these plots above are redrawn by switching the two parameters.

```{r}
ggplot(wine_no_outlier_density_residualsugar, 
       aes(x = quality, fill = residual.sugar_cate)) +
  geom_bar(position = "dodge") +
  labs(fill = "Residual Sugar (g/L)") +
  facet_wrap(~alcohol_cate) +
  ggtitle("Quality by different Alcohol (%) level")  
```


From the two plots above, we can see that apparently higher Quality wines tend to have less Residual Sugar and higher alcohol level. The less Residual Sugar the wine has, the higher Quality it has. The more Alcohol the wine have, the higher Quality it has. And when Residual Sugar is 5g/L and Alcohol is 13%, we have the maximum number of wines with higher Quality.
...
```{r}
wine_no_outlier_totalacidity_chlorides <-
  remove_outliers(wine_no_outlier_totalacidity, chlorides)
```
...
```{r}
wine_no_outlier_totalacidity_chlorides$chlorides_cate <-
  cut(wine_no_outlier_totalacidity_chlorides$chlorides,
      seq(0.015, 0.075, 0.01), labels=seq(0.015, 0.065, 0.01), right=FALSE)
```
...
Here are the plots showing the collective effects on Quality by Chlorides and Total Acidity.

```{r}
ggplot(wine_no_outlier_totalacidity_chlorides, 
       aes(x = quality, fill = chlorides_cate)) +
  geom_bar(position = "dodge") +
  labs(fill = "Chlorides (g/L)") +
  facet_wrap(~total.acidity_cate) +
  ggtitle("Quality by different Total Acidity (g/L) level")
```

Here the plots are redrawn by switching the two parameters.

```{r}
ggplot(wine_no_outlier_totalacidity_chlorides, 
       aes(x = quality, fill = total.acidity_cate)) +
  geom_bar(position = "dodge") +
  labs(fill = "Total Acidity (g/L)") +
  facet_wrap(~chlorides_cate) +
  ggtitle("Quality by different Chlorides (g/L) level")
```
...
```{r}
wine_no_outlier_residualsugar_totalacidity$residual.sugar_cate <-
  cut(wine_no_outlier_residualsugar_totalacidity$residual.sugar,
      seq(0, 25, 5), labels=c(1:5)*5, right=FALSE)
```
...
```{r}
wine_no_outlier_residualsugar_totalacidity$total.acidity_cate <-
  cut(wine_no_outlier_residualsugar_totalacidity$total.acidity,
      seq(4.5, 9.5, 1), labels=seq(5, 9, 1), right=FALSE)
```

Here are the plots showing the collective effects on Quality by Total Acidity and Residual Sugar.

```{r}
ggplot(wine_no_outlier_residualsugar_totalacidity, 
       aes(x = quality, fill = total.acidity_cate)) +
  geom_bar(position = "dodge") +
  labs(fill = "Total Acidity (g/L)") +
  facet_wrap(~residual.sugar_cate) +
...
Here those plots are redrawn by switching the two parameters.
...
In the plots above, we can see that when Total Acidity is 7g/L and Residual Sugar is 5g/L, we have the maximum number of wine with high Quality.
...
library(ggplot2)
ggplot(aes(x = quality), data = wine) +
  geom_bar(color = 'black', fill = I('#099009'), position = "stack") +
  stat_function( fun = function(x, mean, sd, n){n * dnorm(x = x, mean = mean,
                                                          sd = sd)},
                 args = with(wine, c(mean = mean(quality), sd = sd(quality),
                                     n= length(quality))),
                 lwd=1.5) +
  scale_x_continuous(breaks = seq(1, 10, 1)) +
  scale_y_continuous(breaks = seq(0, 2200, 100)) +
  xlab('Quality of Wines') +
  ggtitle("Distribution of Quality of Wines")

```

## Description One
Quality seems to be a normal distribution. Its statistics is below
...
ggplot(aes(y = density, x = quality, group=quality),
       data = wine_no_outlier_density_groupby_quality) + 
  geom_boxplot(color = 'blue', fill = I('#099009'),
               outlier.size = 3, outlier.colour = 'black') +
  geom_smooth(method = "lm", color = "red",
              fill = "yellow", se = TRUE, aes(group=1)) +
